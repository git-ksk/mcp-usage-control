...
v0.6 progressive growth [RELEASED]
 ...
 -> v1.1 post-v1 integration ergonomics [IN PROGRESS]
```
...
| **v1.1.0** | Additive post-v1 integration ergonomics: bounded Cloudflare exact post-reserve retry (#232), MCP read-only operationId design/ergonomics (#233) | In progress |
...
**v1.0.0 is the released feature-free stable-promotion milestone and the five-package npm baseline.** The next bounded development line is **v1.1.0 post-v1 integration ergonomics**: #232 first, then #233. It must remain additive and must not weaken the frozen v1 accounting/replay boundary. The first npm publication gate #6 is complete for `v1.0.0`. Future registry releases remain independently authorized and use the manual OIDC Trusted Publishing workflow; source-release progress does not authorize registry publication.
...
| #232 Cloudflare bounded exact post-reserve retry | v1.1 | **Adopt; explicit opt-in Cloudflare helper, never retry initial reserve** |
| #233 MCP read-only operationId ergonomics | v1.1 | **Design next; preserve logical-operation identity and cross-session isolation** |
