...
| Cloudflare exact post-reserve retry stays bounded, delays retryable replays with equal-jitter backoff, and never broadens retry to initial reserve/vector/growth/conflict paths | `packages/cloudflare/src/exact-retry.test.ts` + packed-subpath release smoke | Node 22/24 + release artifact validation |
| Cloudflare exact-retry telemetry remains privacy-bounded and best-effort; observer failures cannot change retry/accounting outcomes | `packages/cloudflare/src/exact-retry.test.ts` | Node 22/24 |
